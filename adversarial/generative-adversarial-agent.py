...
def main_baseline(args):
    ...
    m_e_conf.update({
        'idim': args.dim, 'odim': args.dim, 'n_neighbors': 5,
    ...
    plt.ion()
    plt.subplot(131)
    plt.title('transfer = %s' % (gs.transf))
    t = np.linspace(-1.2, 1.2, 101)
    plt.plot(t, gs.transf(t))
    ...
    for i in range(1, args.numsteps):
        ...
        nu = np.random.normal(0, 1, size = (args.dim, 1)) * 0.0
        # print "gs.x", gs.x, "nu", nu
        # y = gs.x + nu
        
        X_m_i = np.vstack((X_[...,[i]], X[...,[i-1]]))
        Y_[...,[i]] = np.tanh(2.0 * m_i.predict(X_m_i.T)) + nu
        # print "Y_", Y_[...,[i]]
        ...
        if i % 100 == 0:
            plt.subplot(132)
            plt.gca().clear()
            plt.plot(X_.T, '-o', alpha = 0.5, label = 'X_')
            plt.plot(X.T, '-o', alpha = 0.5, label = 'X')
            plt.plot(Y_.T, '-x', alpha = 0.5, label = 'Y')
            plt.legend()
            plt.subplot(133)
            plt.gca().clear()
            plt.plot(E_min_.T, X_e_min_.T, 'o', alpha = 0.5, label = 'X_e_min')
            plt.plot(E_max_.T, X_e_max_.T, 'o', alpha = 0.5, label = 'X_e_max')
            plt.legend()
            plt.draw()
            plt.pause(1e-9)
